# Figures — Causal-Invariant Digital Twin (CIDT)

This directory contains **all primary figures used in the paper**  
*Causal-Invariant Digital Twin for Robust Autonomous Decision Validation under 5G-V2X Distribution Shifts*.

All figures are embedded below to ensure **inline rendering on GitHub** and
easy navigation for reviewers.

---

## 🧠 Framework & Causal Modeling

### Causal-Invariant Digital Twin (CIDT) Architecture
![CIDT Architecture](./cidt.png)

### Structural Causal Twin Model
![Structural Causal Twin](./structural_causal_twin_model.png)

### Learned Causal Graph
![Causal Graph](./causal_graph.png)

---

## 🔁 Causal Effects & Decomposition

### Causal Effect Decomposition
![Causal Effect Decomposition](./causal_effect_decomposition.png)

### Loss Decomposition Analysis
![Loss Decomposition](./loss_decomposition.png)

These figures illustrate how CIDT separates invariant causal effects
from spurious correlations.

---

## 📊 Baseline vs Robust vs Causal Comparisons

### ERM vs IRM
![ERM vs IRM](./erm-irm.png)

### DANN Performance
![DANN](./dann.png)

### GroupDRO Performance
![GroupDRO](./groupdro.png)

### Predictive vs Invariant Models (All Metrics)
![Predictive vs Invariant](./predictive_vs_invariant_all_metrics.png)

---

## ⚠️ Fault Modeling & Event Analysis

### SEFN Architecture
![SEFN](./sefn.png)

### Radar + Event Importance (SEFN Ablation)
![SEFN Event Importance](./sefn_ablation_radar_plus_event_importance.png)

### Fault Classification Comparison
![Fault Classification](./fault_classification_comparison.png)

---

## ⚖️ Ethical AI & Fairness

### Group-wise Fairness Metrics
![Fairness Metrics](./fairness_NxN_all_metrics.png)

---

## 🌍 Dataset & Environment Analysis

### Environment Distribution (with Legend)
![Environment Distribution](./env_distribution_by_name_WITH_LEGEND.png)

---

## 📈 Sample Efficiency & Convergence

### AUROC Convergence vs Sample Size
![Sample Efficiency](./sample_efficiency_auroc_convergence.png)

---

## 📊 Aggregated Results

### Section G — Grouped Bar Results
![Grouped Bar Results](./sectionG_grouped_bar_results.png)

---

## 📌 Figure Inventory

| Category | Files |
|-------|------|
| Framework | `cidt.png`, `structural_causal_twin_model.png` |
| Causal | `causal_graph.png`, `causal_effect_decomposition.png` |
| Robust ML | `erm-irm.png`, `dann.png`, `groupdro.png` |
| Fault Modeling | `sefn.png`, `sefn_ablation_radar_plus_event_importance.png` |
| Fairness | `fairness_NxN_all_metrics.png` |
| Dataset | `env_distribution_by_name_WITH_LEGEND.png` |
| Efficiency | `sample_efficiency_auroc_convergence.png` |
| Aggregates | `sectionG_grouped_bar_results.png` |

---

## 📖 Usage in Paper

These figures are referenced across:
- **Sections A–L** (Results & Ablations)
- Main framework and methodology sections
- Ethical AI and robustness evaluation

All figures are final, publication-quality, and aligned with IEEE Transactions standards.

